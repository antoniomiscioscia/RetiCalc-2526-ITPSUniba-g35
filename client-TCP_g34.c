...
#include <arpa/inet.h>
#include <unistd.h>
#include <netdb.h>                                                                  // Per gethostbyname() [cite: 311]
...
int main(void) {
    ...
    printf("Inserisci il nome del server (es. localhost, 127.0.0.1, www.uniba.it):\n");
    scanf("%s", serverName);

    // 3. CLIENT: risoluzione del nome
    if ((host = gethostbyname(serverName)) == NULL) {                               // Risoluzione da nome simbolico a IP [cite: 253]
        fprintf(stderr, "Risoluzione del nome fallita per %s.\n", serverName);    // Se la risoluzione fallisce, terminare il client
        ClearWinSock();
        return EXIT_FAILURE;
    }
    // L'indirizzo IP è ora in host->h_addr_list[0] [cite: 268]
    ...
    if ((Csocket = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {
        ...
        ErrorHandler("Creazione della socket fallita.\n");                          // Se la creazione fallisce, terminare il client
        ClearWinSock();                                                             
    ...
    if (connect (Csocket, (struct sockaddr *)&sad, sizeof(sad)) < 0) {
        ErrorHandler( "Connessione fallita.\n");                                      // Se la connessione fallisce, termina il client
        closesocket(Csocket);                                                       // Chiude la socket
        ClearWinSock(); 
        return EXIT_FAILURE;                                                        // Restituisce 0 in caso di successo, altrimenti -1
    }
    ...
    if (send(Csocket, &operation_char, 1, 0) != 1) {                                // Invia la lettera al server
    ...
    if (recv(Csocket, response_string, ECHOMAX - 1, 0) <= 0) {                    // Riceve la stringa di risposta
    ...
    else 
    {
        ...
        // Invia i due operandi
        long operands[2];
        operands[0] = htonl((long)op1);                                                 // Conversione Host to Network Long
        operands[1] = htonl((long)op2);
        
        // 8. CLIENT: spedisce i due interi (2 * sizeof(long) bytes)
        if (send(Csocket, (char *)operands, sizeof(long) * 2, 0) != sizeof(long) * 2) {
        ...
        // 10. CLIENT: riceve il risultato (1 * sizeof(long) bytes)
        long net_result;
        if (RecvExact(Csocket, (char *)&net_result, sizeof(long)) <= 0) {
